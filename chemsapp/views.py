...
def wall_chart_pdf(request):
    ...
    # Support both product_ids and product_variant_ids
    ...
    if variant_ids:
        # Resolve variants to products, preserving order, deduping
        seen = set()
        ordered_product_ids = []
    ...
    # Preserve the order specified by the caller
    products_by_id = {
        p.pk: p
    ...
    # Build per-product QR codes for SDS sheets; strip HTML from rich text fields
    import pyqrcode
    from django.utils.html import strip_tags
    import html as html_module
    ...
    for product in products:
        ...
        # Strip HTML tags and decode entities for clean plain text in PDF
        ...
        # Compute a darker stripe colour (60% brightness of the row colour)
    ...
    # Resolve logo path — check source static dir first, then collected static
